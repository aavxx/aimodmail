...
The DM hook must read `installed` and `confirm_thread_creation` must read `off`.
...
Shown when a user declines the privacy notice, or when thread creation is
otherwise stopped.
...
|----------------|------------------------------------------------------------|-----------|
| `consent`      | `user_id`, `accepted_at`, `policy_version`                   | kept      |
